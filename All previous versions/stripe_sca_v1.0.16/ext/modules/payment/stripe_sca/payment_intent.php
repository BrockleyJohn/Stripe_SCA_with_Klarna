...
if (isset($token) && $token !== '0') {
    $token_query = tep_db_query("select stripe_token from customers_stripe_tokens where id = '" . (int) $token . "' and customers_id = '" . (int) $customerId . "'");

    if (tep_db_num_rows($token_query) === 1) {
        $rec = tep_db_fetch_array($token_query);
...
if (!isset($id)) {
    ...
    $stripe_sca->event_log($customer_id, "ajax retrieve", $id, $pi);
    if (isset($token) && $token > 0) {
